...
export default function ContactForm() {
  ...
  const onSubmit: SubmitHandler<Inputs> = (payload) => {
    ...
    setSending(true);
    fetch("https://rps.agh.edu.pl/mailer.php", {
      method: "POST",
      headers: {
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          alert("Wystąpił błąd podczas wysyłania wiadomości, skontaktuj się z nami w inny sposób");
        }
      }
      )
      .catch((err) => {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-2 flex flex-col">
        <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-4">
          <TextInput
            ...
            disabled={sending || hasSentSuccessfully}
            icon={(
        ...
        <button
          type="submit"
          disabled={sending || hasSentSuccessfully}
          className={`mt-3 self-end flex justify-center w-24 text-white ${hasSentSuccessfully ? "bg-green-700" : "bg-brand-600 hover:bg-brand-700"} focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-md text-sm px-5 py-2.5 text-center`}
        >
          {
            sending ? (
